...
from marketAdmin.models import medicines, health
...
class AddtoCart(View):
    ...
    def get(self, request,id,price,name):
        ...
        if 'name' not in request.session and  'email' not in request.session:
            ...
            myitemv = health.objects.filter(Q(hname=name) & Q(hprice=price) & Q(id=id))
   
            myitemg = medicines.objects.filter(Q(mname=name) & Q(mprice=price) & Q(id=id))
        ...
        # print(getattr(myitemg, myitemg.other_field))
            ...
            if myitemg.count() != 0:
                ...
                return HttpResponseRedirect('/market/medicines')
            if myitemv.count() != 0:
                for i in myitemv:
                    name = i.hname
                    price = i.hprice
                ...
                cart.objects.create(uid = Users.objects.get(email=request.session['email']),item_name =name,item_price =price).save()
                messages.info(request,'Item sdded successfully to cart')
                return HttpResponseRedirect('/market/health')
...
def DRCTBuyFrmcrd(request,id):
    if 'name' in request.session and 'email' in request.session:
        ...
        myItems = health.objects.get(pk=id)
        
        return render(request,'users/singlebuy.html',{'userdata':userdata,'myItems':myItems,'health':False})
...
def DRCTBuyFrmcrdGroce(request,id):
    if 'name' in request.session and 'email' in request.session:
        ...
        myItems = medicines.objects.get(pk=id)
        

           
       
        
        return render(request,'users/singlebuy.html',{'userdata':userdata,'myItems':myItems,'health':True})
